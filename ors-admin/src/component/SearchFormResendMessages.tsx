...
export const SearchFormResendMessages: React.FC<SearchComponentOptions> = ({
  handleFunction,
  handleResendAll,
  messages,
  width
      ...
      <div style={{marginBottom: '18px'}}>
        <div>
          <form onSubmit={handleSubmit(onSubmit)}>
            <InlineFieldRow>
            <InlineField label="AutoId">
                <Input
                  width={16}
                  css={''}
                  value={data.autoId}
                  onChange={e => handleOnchange(e, 'autoId')}
                />
              </InlineField>
              <InlineField label="MsgId">
                <Input
                  width={16}
                  css={''}
                  value={data.msgId}
                  onChange={e => handleOnchange(e, 'msgId')}
                />
              </InlineField>
          ...
          {Constant.getInstance().canEdit && (
                <div disabled={!active} style={{position: 'absolute', top:'90px', left: 'calc(100% - 110px)' }}>
                  <Button
                    type="button"
